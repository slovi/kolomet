...
public class ProductController extends AbstractController {
	...
	public String listByCategory(@PathVariable("categoryCodeKey") String categoryCodeKey, Model model) {
		...
		if (!categoryCodeKey.equals(Category.ALL_CATEGORY_CODE_KEY)) {
			model.addAttribute("products", productRepository.findByCategory(categoryCodeKey, new PageRequest(0, 10)));
		} else {
			model.addAttribute("products", productRepository.findByPriority(new PageRequest(0, 10)));
		}
		return "products/list_category";
	}
	...
	public String listByProducer(@PathVariable("producerId") Long producerId, Model model) {
		...
		model.addAttribute("products", productRepository.findByProducerOrderByCreatedDesc(producer, new PageRequest(0, 10)));
		return "products/list_category";
	}
	...
	public String listByCategoryType(@PathVariable("categoryTypeCodeKey") String categoryTypeCodeKey, Model model) {
		...
		model.addAttribute("products", productRepository.findByCategoryType(categoryTypeCodeKey, new PageRequest(0, 10)));		
		return "products/list_category";
	}
